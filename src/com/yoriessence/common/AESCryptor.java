package com.yoriessence.common;
...
//javax.crypto패키지, java.security패키지에 구성되어 있음.

//key관리가 중요함! key는 프로젝트에서 한개만 있어야함.
public class AESCryptor {
	...
	public AESCryptor() {
		...
		File f=new File(this.path+"/yorie.cg");
		//key를 저장하고 있는 파일이름이 bslove.bs -> SecretKey객체가 저장되어있음
		if(f.exists()) {
			//key를 저장하는 파일이 있으면..?
					...
					BadPaddingException, IllegalBlockSizeException
	{
		...
		//매개변수로 넘어온 값 암호화처리하기
		byte[] encrpty=str.getBytes(Charset.forName("utf-8"));
		byte[] result=cipher.doFinal(encrpty);
