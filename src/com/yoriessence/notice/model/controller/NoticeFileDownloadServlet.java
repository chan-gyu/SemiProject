...
public class NoticeFileDownloadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//3.클라이언트에게 보낼 파일명 인코딩처리하기 -> 한글파일명이 꺠지지않게
		...
		//응답메세지작성
		response.setContentType("application/octet-stream");//알려지지않은타입일떄씀
		response.setHeader("Content-disposition", "attachment;filename="+fileRename);
		
		//클라이언트에게 보낼 스트림 열기
		ServletOutputStream sos=response.getOutputStream();
		BufferedOutputStream bos=new BufferedOutputStream(sos); //그냥보내면 느리니까 버퍼씌움
		
		int read=-1; //아무의미 없는 값 (임의의 값)
